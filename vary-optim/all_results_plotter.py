...
for optimiser in optimisers:
    ...
    df[optimiser]['val'] = pickle.load(open(f"./vary-optim/pickles/val_loss_{optimiser}.pkl", 'rb'))
...
for optimiser in optimisers:
    for phase in df[optimiser].keys():
        if phase == 'val':
            plt.plot(df[optimiser][phase], label=f"{optimiser} {phase}",linestyle='dashed',color=colours[list(df.keys()).index(optimiser)])
        else:
            plt.plot(df[optimiser][phase], label=f"{optimiser} {phase}",color=colours[list(df.keys()).index(optimiser)])
